...
**Always-on API (M5.5.2 / M5.6.1):**

| Node | Install | launchd label |
|------|---------|---------------|
| M5 dev | `./scripts/install-workspace-api-launchd.sh` | `pl.octadecimal.workspace-api-dev` |
| M1 server | `./scripts/install-workspace-api-m1-launchd.sh` | `pl.octadecimal.workspace-api-m1` |

Do not install both labels on one Mac (same port `:8042`). See [daily dev runbook](../runbooks/workspace-daily-dev.md) (M5) and [M1 server mode](../runbooks/workspace-m1-server-mode.md).
...
- Full parity with workspace.octadecimal.pro design system — **frontend-team** ([triple-track](../planning/workspace-mvp-triple-track.md))
- Security Ops (Lynis → HITL remediacja) — **security-team** ([M5.8](../planning/workspace-mvp-m5-8-security-ops.md))
...
| [M5.8](../planning/workspace-mvp-m5-8-security-ops.md) | parallel | security-team | Audit → remediate → HITL |
| [M5.7](../planning/workspace-mvp-m5-7-hosting-only.md) | deferred | backend-team | pc-ubuntu hosting only (no HYDRA agents) |
| UX.1–UX.5 | parallel | frontend-team | Design system, responsive UI — [triple-track](../planning/workspace-mvp-triple-track.md) |
